...
if not WAYFORPAY_MERCHANT or not WAYFORPAY_SECRET:
    raise RuntimeError("WAYFORPAY_MERCHANT or WAYFORPAY_SECRET missing")
if not SERVICE_URL:
...
def wfp_invoice_signature(payload: dict) -> str:
    ...
    msg = ";".join(parts)
    return hmac.new(WAYFORPAY_SECRET.encode(), msg.encode(), hashlib.md5).hexdigest()
...
def wfp_callback_valid(body: dict) -> bool:
    ...
    msg = ";".join(parts)
    expected = hmac.new(WAYFORPAY_SECRET.encode(), msg.encode(), hashlib.md5).hexdigest()
...
async def pay_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.get("reasonCode") not in (1100, 1101, 1102):  # 1100–успіх, 1101/1102 – теж ок для інвойсу
        await query.message.reply_text("Помилка при створенні інвойсу.")
        return
