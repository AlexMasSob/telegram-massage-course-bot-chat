...
async def wfp_callback(request: Request):
    ...
    # Тут ми не можемо однозначно визначити telegram_id з orderReference,
    # бо використовується статична кнопка. Тому просто приймаємо callback
    # і відповідаємо WayForPay коректним підписаним статусом.
    ts = int(time.time())
